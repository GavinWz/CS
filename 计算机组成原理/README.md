...
3. 中断向量地址形成部件
    ...
    * 向量地址：中断向量保存的内存单元的地址
    ...
    排队器的输出（仅有一个高电平）作为该部件的输入，输出对应中断向量地址，指出了中断服务程序的入口地址或跳转指令的地址
...
3. 主程序和服务程序抢占CPU示意图
    ![主程序和服务程序抢占CPU](./Pictures/主程序和服务程序抢占CPU.png)
...
2. 浮点表示
    ...
    * r为尾数的基值
    ...
    在计算机中首先规定尾数的基值，然后分别规定浮点数的阶码、尾数的符号和位数
...
2. 算数移位的硬件实现
    ...
    ![算数移位的硬件实现](./Pictures/算数移位的硬件实现.png)
